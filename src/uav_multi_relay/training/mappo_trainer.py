...
def _observation_arrays(observation: object) -> tuple[np.ndarray, np.ndarray]:
    ...
    if local.ndim != 2 or global_state.ndim != 1 or not np.all(np.isfinite(local)) or not np.all(np.isfinite(global_state)):
        raise ValueError("observation arrays must be finite with ranks 2 and 1")
...
def train_mappo(
    env: MultiRelayEnvironment, agent: MAPPOAgent, config: MAPPOTrainingConfig, *,
    ...
    episode_returns: list[float] = []; episode_lengths: list[int] = []
    rates: list[float] = []; completed_episodes = terminated_episodes = interventions = mismatch_events = 0
    current_return = 0.0; current_length = 0; total_updates = 0; last_metrics: MAPPOUpdateMetrics | None = None
    for index in range(config.total_environment_steps):
        requested, old_log_probability, value = agent.act_with_stats(local, global_state)
        ...
        rollout.add(local, global_state, requested, applied, old_log_probability, reward, value, next_value, terminated, truncated)
        rate = float(info["rate_e2e_bps"]); intervention_norms = np.asarray(info["intervention_norms"], dtype=float)
        ...
        rates.append(rate); interventions += int(np.any(intervention_norms > 1e-9)); mismatch_events += int(np.any(mismatch > 1e-6))
        current_return += float(reward); current_length += 1
        if rollout.is_full:
            last_metrics = agent.update(rollout); total_updates += 1; rollout.clear()
        local, global_state = next_local, next_global
        if terminated or truncated:
            episode_returns.append(float(current_return)); episode_lengths.append(current_length); completed_episodes += 1; terminated_episodes += int(terminated)
            current_return = 0.0; current_length = 0
        ...
        if progress_callback is not None and (steps % int(progress_interval_steps) == 0 or steps == config.total_environment_steps):
            progress_callback(MAPPOTrainingProgress(
                steps, total_updates, completed_episodes, float(np.mean(rates)),
                float(terminated_episodes / completed_episodes) if completed_episodes else 0.0,
                float(interventions / steps), float(mismatch_events / steps), last_metrics,
            ))
    values = [float(np.mean(rates)), float(terminated_episodes / completed_episodes) if completed_episodes else 0.0, float(interventions / config.total_environment_steps), float(mismatch_events / config.total_environment_steps), *episode_returns]
    if not all(np.isfinite(value) for value in values):
        raise ValueError("training summary contains non-finite values")
    return MAPPOTrainingSummary(config.total_environment_steps, total_updates, completed_episodes, tuple(episode_returns), tuple(episode_lengths), values[0], values[1], values[2], values[3], last_metrics)
