...
def main() -> None:
    ...
    parser.add_argument("--policies", nargs="+", default=["masac", "random", "stationary", "equal_spacing", "greedy", "mpc"])
    ...
    with (output / "comparison_config.json").open("w", encoding="utf-8") as handle:
        json.dump({"comparison_config": asdict(config), "agent": {"num_relays": agent.num_relays, "local_observation_dim": agent.local_observation_dim, "global_state_dim": agent.global_state_dim, "action_dim": agent.action_dim}, "max_steps": environment_config.max_steps}, handle, allow_nan=False, indent=2)
